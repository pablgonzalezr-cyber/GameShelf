...
# 2. Integrantes
Integrantes
Pablo González     -----> pablgonzalezr-cyber
Gabriel Chodil     -----> GabrielChodil
...
Registrar roles.
Registrar usuarios asociados a un rol válido.
Registrar categorías de videojuegos.
Registrar videojuegos asociados a una categoría activa.
Registrar stock asociado a videojuegos existentes.
Crear préstamos y reducir automáticamente el stock disponible.
Devolver préstamos y aumentar automáticamente el stock disponible.
Crear reservas y registrar historial de cambios.
Crear y pagar multas asociadas a préstamos.
Crear y leer notificaciones asociadas a usuarios.
Crear autorizaciones y validar permisos correctos e incorrectos.
---
...
---
Requisitos previos
Para ejecutar y revisar el proyecto correctamente se requiere:
Java JDK compatible con el proyecto.
Maven instalado y disponible desde terminal.
Docker Desktop iniciado para la versión Docker.
XAMPP con MySQL configurado en el puerto `3307` para la versión nativa/local.
Postman o herramienta equivalente para probar el flujo funcional por API Gateway.
Navegador web para revisar Eureka Server y Swagger/OpenAPI.
...
Ejecución nativa mediante script `.bat`
La versión nativa descargable debe incluir los archivos `.jar` compilados y un script de arranque para Windows.
Pasos recomendados:
Descargar la versión nativa desde el enlace indicado al inicio del README.
Descomprimir el archivo `.zip`.
Verificar que XAMPP esté iniciado con MySQL en el puerto `3307`.
Ejecutar el archivo:
```bat
arrancar-sistema.bat
```
El script debe iniciar los componentes respetando el orden jerárquico:
```text
1. Eureka Server
2. Microservicios de negocio
3. API Gateway
```
Una vez iniciado el sistema, abrir:
```text
...
Genera los `.jar`.
Genera reportes de cobertura JaCoCo en cada microservicio configurado.
...
El proyecto incorpora pruebas unitarias y pruebas de controller en los microservicios de negocio.
...
Las pruebas unitarias se dividen principalmente en dos tipos:
Service Test: validan reglas de negocio, validaciones internas, manejo de excepciones, uso de repositories simulados y comunicación simulada con otros microservicios mediante Feign Client.
Controller Test: validan endpoints REST, métodos HTTP, códigos de respuesta, JSON enviado, JSON retornado y manejo de errores mediante MockMvc.
En los microservicios con comunicación externa se simulan los Feign Client con Mockito para evitar depender de otros servicios reales durante las pruebas.
...
true
```
Validación incorrecta con permiso permitido, pero no asignado:
```json
{
  "usuarioId": 1,
  "modulo": "PRESTAMOS",
...
false
```
> Nota: se utiliza `TESTEADOR` como ejemplo de permiso incorrecto porque es un permiso permitido por el sistema. No se recomienda usar `ELIMINAR\_PRESTAMOS` para esperar `false`, porque si ese permiso no existe dentro de la lista de permisos válidos, el sistema puede responder `400 Bad Request` por permiso inválido.
---
